...
async function listiraamatud() {
    const responseData = await getDataAsJson("https://ggdocker.azurewebsites.net/raamatud/");
    const resultElement = document.getElementById("raamatud_result");
    resultElement.innerHTML = ""
    for (var raamat of responseData.raamatud){
        resultElement.innerHTML += '<a href="https://ggdocker.azurewebsites.net/raamatud/'+raamat+'"  download="'+raamat+'.txt" >' +raamat+".txt</a> " +
            '<a href="#" onclick="deleteObject(\'https://ggdocker.azurewebsites.net/raamatud/'+raamat+'\')" > [kustuta]</a>' +
            "<br />";
    }
